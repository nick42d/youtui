...
impl<S> AsRef<[u8]> for DroppableSong2<S>
where
    S: AsRef<[u8]> + Send + Sync + 'static,
{
    fn as_ref(&self) -> &[u8] {
...
impl<S> Drop for DroppableSong2<S>
where
    S: AsRef<[u8]> + Send + Sync + 'static,
{
    fn drop(&mut self) {
...
pub struct AsyncRodio {
    handle: tokio::task::JoinHandle<()>,
    tx: tokio::sync::mpsc::Sender<AsyncRodioMessage>,
}

impl Drop for AsyncRodio {
    fn drop(&mut self) {
        self.handle.abort()
    }
}
...
fn add_periodic_access<S>(song: Decoder<Cursor<S>>) -> DroppableSong2<Decoder<Cursor<S>>>
where
    S: AsRef<[u8]> + Send + Sync + 'static,
{
    song.map(|s| DroppableSong2 {
        song,
        song_id: todo!(),
        channel: todo!(),
    })
...
impl AsyncRodio {
    pub fn play_song(&self, song: impl AsRef<[u8]>) -> impl Stream<Item = ()> {}
