...
impl AppRequest {
    fn category(&self) -> RequestCategory {
        match self {
            ...
            AppRequest::IncreaseVolume(..) => RequestCategory::Volume,
            AppRequest::GetVolume => RequestCategory::Volume,
        }
    }
}
    ...
    pub async fn send_request(&mut self, request: AppRequest) -> Result<()> {
        ...
        match context {
            TaskContext {
                ...
                self.request_tx
                    .send(server::Request::Api(server::api::Request::NewArtistSearch(
                        s,
            ...
            TaskContext {
                ...
                self.request_tx
                    .send(server::Request::Api(
                        server::api::Request::GetSearchSuggestions(
                            s,
                            KillableTask { id, kill_rx: rx },
                        ),
                    ))
            ...
            TaskContext {
                ...
                self.request_tx
                    .send(server::Request::Api(
                        server::api::Request::SearchSelectedArtist(
                            s,
                            KillableTask { id, kill_rx: rx },
                        ),
                    ))
            ...
            TaskContext {
                ...
                self.request_tx
                    .send(server::Request::Downloader(
                        server::downloader::Request::DownloadSong(video_id, playlist_id, id, rx),
                    ))
                    .await?
            }
            TaskContext {
                request: AppRequest::GetVolume,
                ..
            } => {
                self.request_tx
                    .send(server::Request::Player(server::player::Request::GetVolume))
                    .await?
            }
            TaskContext {
                request: AppRequest::IncreaseVolume(i8),
                ..
            } => {
                self.request_tx
                    .send(server::Request::Player(
                        server::player::Request::IncreaseVolume(i8),
                    ))
                    .await?
            }
        };
